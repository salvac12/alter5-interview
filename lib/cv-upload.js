...
const { getPositionById, getPositionBySlug } = require('./positions');
...
async function processCvUpload({
  ...
  headhunterId = null,
  positionId = null,        // explicit position chosen by uploader (admin / headhunter)
  positionSlug = null,      // alternative way to pin position (e.g. 'hoe' fallback)
  uploaderNote = null,
  autoInvite = false,
  ip = null,
  userAgent = null,
  ...
  // Resolve target position FIRST — its prompt drives both the email-extraction
  // prefetch and the final scoring. Caller passes id OR slug; default to 'hoe'
  // so legacy callers (admin_manual w/o picker, email_agent) still work.
  let position = null;
  if (positionId) position = await getPositionById(positionId);
  if (!position && positionSlug) position = await getPositionBySlug(positionSlug);
  if (!position) position = await getPositionBySlug('hoe');
  if (!position) {
    return { ok: false, status: 500, error: 'no_default_position' };
  }
  if (position.status !== 'active') {
    return { ok: false, status: 400, error: 'position_inactive', detail: position.slug };
  }

  // Resolve email — prefilled wins; otherwise extract from CV using the
  // position's prompt so the score we cache for this prefetch reflects this
  ...
  if (!resolvedEmail) {
    analysisPrefetched = await analyzeCv({
      fileBase64,
      filename,
  ...
  const { data: existing, error: findErr } = await supabaseAdmin
    .from('applications')
    .select('id, email, name, experience, source, headhunter_id, position_id')
    .eq('email', normEmail)
    .is('deleted_at', null)
    .maybeSingle();
  ...
  // 3. Analyze (or reuse the email-extraction prefetch — both use the same
  //    position prompt so the score is consistent).
  const analysis = analysisPrefetched || await analyzeCv({
    fileBase64,
    filename,
    systemPrompt: position.cv_analysis_prompt,
  });
