// Resend wrapper.
//
// Templates (all branded against the Alter5 design tokens):
//   sendMagicLinkEmail              - verify email + upload CV
//   sendInterviewLinkEmail          - invitation to the technical interview
//   sendInterviewReminderEmail      - nudge approved candidates who never
//                                     opened or didn't finish the interview
...
// CRITICAL: use single quotes inside the value — these strings are inlined
// into style="..." attributes, and double quotes there collide with the
// attribute delimiter, truncating the entire style (Gmail/Outlook render
// every affected element unstyled). CSS accepts ' and " interchangeably
// for quoted family names, so this is a drop-in swap.
...
// Default subtitle — shown when the caller didn't pass a positionTitle
// (e.g. partner-invite emails). Funnel emails now pass the actual position
// title so the candidate sees the exact role at the top of the message.
//
// `subtitle` is escaped here because it can flow through from DB (position
// title/subtitle). A title like "Ops <staff>" would otherwise inject HTML
// into every outgoing email.
//
// The logo is a PNG (not SVG) because Outlook + a handful of Gmail variants
// still drop inline SVG. We serve it from careers.alter-5.com/ds so images
// live behind the same domain the email links to — some clients flag
// cross-domain image hosts. width/height attributes are set on the <img>
// itself (not only via style) because Outlook ignores width in CSS.
function brandHeader(subtitle = 'Proceso de selección') {
  ...
  <tr><td style="padding:32px 40px">
    <img src="https://careers.alter-5.com/ds/alter5-logo-white.png"
         alt="Alter5"
         width="140" height="33"
         style="display:block;border:0;outline:none;text-decoration:none;-ms-interpolation-mode:bicubic;height:auto;max-width:140px" />
    <div style="font-family:${FONT};font-size:11px;color:${C.textDim};letter-spacing:0.14em;text-transform:uppercase;margin-top:14px">
      ${esc(subtitle)}
    </div>
  </td></tr>
</table>`;
...
function wrap(bodyHtml, { subtitle } = {}) {
  return `<!doctype html>
<html lang="es">
<head>
  <meta charset="utf-8">
  ...
  <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="background:${C.pageBg};padding:32px 16px">
    <tr><td align="center">
      <table role="presentation" width="600" cellpadding="0" cellspacing="0" border="0" style="max-width:600px;width:100%;background:${C.white};border-radius:12px;overflow:hidden;box-shadow:0 4px 20px rgba(19,40,91,0.08)">
        <tr><td>${brandHeader(subtitle)}</td></tr>
...
function sendMagicLinkEmail({ to, verifyUrl, ttlMinutes = 30, positionTitle = null }) {
  const safePos = esc(positionTitle) || 'la posición publicada';
  const body = `
    <p style="${P_LEAD}">Hola,</p>
    <p style="${P_BODY}">
      Recibimos tu candidatura a <strong style="color:${C.text}">${safePos}</strong>.
    </p>
    <p style="${P_BODY_END}">
  ...
  return sendEmail({
    ...
    html: wrap(body, { subtitle: positionTitle || undefined }),
  });
}

function sendInterviewLinkEmail({ to, name, interviewUrl, expiresDays = 7, positionTitle = null }) {
  ...
  const body = `
    <p style="${P_LEAD}">${hi}</p>
    <p style="${P_BODY}">
      Tu perfil encaja con lo que buscamos para <strong style="color:${C.text}">${safePos}</strong>.
    </p>
    <p style="${P_BODY_END}">
...
// Nudge for candidates who were approved (CV) but didn't open or didn't
// finish the interview. Copy intentionally differs from sendInterviewLinkEmail
// (the initial "your profile fits" invite) — that phrasing would feel
// redundant or pushy in a reminder. Admin triggers this from /admin; each
// click issues a fresh magic link and marks prior unused ones as used, so
// the link here is always live.
function sendInterviewReminderEmail({ to, name, interviewUrl, expiresDays = 7, positionTitle = null }) {
  const safeName = esc(name);
  const safePos = esc(positionTitle) || 'la posición publicada';
  const hi = safeName ? `Hola ${safeName},` : 'Hola,';
  const body = `
    <p style="${P_LEAD}">${hi}</p>
    <p style="${P_BODY}">
      Queríamos recordarte que aún tienes pendiente la entrevista técnica para
      <strong style="color:${C.text}">${safePos}</strong>.
    </p>
    <p style="${P_BODY_END}">
      Te llevará unos <strong style="color:${C.text}">15 minutos</strong>.
      Hemos generado un enlace nuevo que caduca en
      <strong style="color:${C.text}">${expiresDays} días</strong>. Si ya la habías empezado,
      puedes retomarla desde el principio con este enlace.
    </p>
    ${cta(interviewUrl, 'Completar entrevista →')}
    <p style="${P_FINE}">
      Si ya no estás interesado en el proceso, ignora este mensaje.
    </p>
    <p style="${P_SIGN}">Un saludo,<br><strong style="color:${C.text}">Equipo Alter5</strong></p>`;
  return sendEmail({
    to,
    subject: `Alter5 — Recordatorio: tu entrevista sigue abierta${safeName ? ' · ' + safeName : ''}`,
    html: wrap(body, { subtitle: positionTitle || undefined }),
  });
}
...
function sendPostCvRejectionEmail({ to, name, positionTitle = null }) {
  const safeName = esc(name);
  const safePos = esc(positionTitle) || 'Head of Engineering (AI &amp; Infrastructure)';
  const hi = safeName ? `Hola ${safeName},` : 'Hola,';
  const body = `
    <p style="${P_LEAD}">${hi}</p>
    <p style="${P_BODY}">
      Gracias por tu interés en la posición de <strong style="color:${C.text}">${safePos}</strong> en Alter5
...
function sendPostInterviewRejectionEmail({ to, name, positionTitle = null }) {
  const safeName = esc(name);
  const safePos = esc(positionTitle) || 'Head of Engineering (AI &amp; Infrastructure)';
  const hi = safeName ? `Hola ${safeName},` : 'Hola,';
  const body = `
    <p style="${P_LEAD}">${hi}</p>
    <p style="${P_BODY}">
      Gracias por haber completado la entrevista técnica para <strong style="color:${C.text}">${safePos}</strong> en Alter5.
...
// One-off rescue mail for candidates whose /api/apply attempt failed due to
// a backend incident (DB constraint, missing env var, etc). We tell them the
// truth (the attempt was NOT stored), ask them to retry, and link to the
// apply page. Kept as a proper template because outages recur.
function sendOutageApologyEmail({ to, applyUrl, positionTitle = null }) {
  const safePos = esc(positionTitle) || 'la posición publicada';
  const body = `
    <p style="${P_LEAD}">Hola,</p>
    <p style="${P_BODY}">
      Ayer intentaste enviar tu candidatura a <strong style="color:${C.text}">${safePos}</strong>
      en Alter5 y, por un fallo técnico en nuestros servidores que ya hemos resuelto,
      tu solicitud no llegó a registrarse. Tus datos no se guardaron.
    </p>
    <p style="${P_BODY_END}">
      Lamentamos mucho las molestias. Si te sigue interesando la posición,
      puedes completar tu candidatura ahora en menos de un minuto:
    </p>
    ${cta(applyUrl, 'Completar candidatura →')}
    <p style="${P_SIGN}">
      Gracias por tu paciencia y por tu interés en Alter5.<br>
      Un saludo,<br>
      <strong style="color:${C.text}">Equipo Alter5</strong>
    </p>`;
  return sendEmail({
    to,
    subject: 'Alter5 — Disculpa, tu candidatura no llegó a registrarse',
    html: wrap(body, { subtitle: positionTitle || undefined }),
  });
}
...
function sendHeadhunterInviteEmail({ to, inviteUrl, ttlHours = 168 }) {
  ...
  const body = `
    ...
    <p style="${P_BODY}">
      ...
      A través de tu portal privado podrás enviarnos candidatos para nuestras posiciones abiertas
      de forma directa.
    </p>
    <p style="${P_BODY_END}">
  ...
  return sendEmail({
    ...
    html: wrap(body, { subtitle: 'Portal de Partners' }),
  });
}
